...
function calculateJulianDate(year:number, month:number, day:number) {
  if (month <= 2) {
    year--;
    month += 12;
  }
...
function calculateTrueNewMoon(k:number) {
  const T = k / 1236.85;
  const T2 = T ** 2;
  const T3 = T ** 3;
  const rad = Math.PI / 180;

  let jde = BASE_NEW_MOON_JD + SYNODIC_MONTH * k + 0.00015437 * T2 - T3 * (0.000000150 + 0.00000000073);
...
function julianDateToUTC(JD:number) {
  JD += 0.5;
...
const determineChineseZodiac = (date:Date) => {
